...
def send_log_to_buffer_with_interval(file_path, buffer, interval=0):
...
def start_log_reader_with_interval(buffer, interval=0):
...
def moving_average_():
    ...
    while(True):
        feature_event.wait()
        timeSeries_anomaly = detector.process_last_line_from_file(file_path)
        print(timeSeries_anomaly)
        feature_event.clear()
